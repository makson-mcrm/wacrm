...
Każde źródło ma trasę `BANK → PRODUKT → TYP ŹRÓDŁA → WERSJA/DATA` oraz jedną z ocen: `POTWIERDZONE`, `CZĘŚCIOWE`, `WNIOSEK AI`, `WYMAGA WERYFIKACJI`.
...
- Odpowiedź jest `POTWIERDZONE` dopiero wtedy, gdy ma zgodne źródło oficjalne i wewnętrzne z dozwolonego folderu Drive.
...
Do przeglądarki trafiają tylko metadane potrzebne do wskazania źródła. Treść dokumentu, identyfikator pliku i prywatny URL nie są zwracane. Pełne pobieranie i indeksowanie Drive należy do części 2 i wymaga wskazania konkretnych folderów oraz połączenia z minimalnym zakresem uprawnień.
