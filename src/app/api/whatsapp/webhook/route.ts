...
export async function GET(request: Request) {
  try {
    ...
    // Stage-one production verification can be provisioned before an access
    // token/App Secret is available. Only its SHA-256 digest is stored in a
    // service-role-only table; the actual token never enters the repository.
    const envToken = process.env.WHATSAPP_VERIFY_TOKEN;
    const envHash = envToken ? hashWebhookVerifyToken(envToken) : null;
    const { data: verificationRows, error: verificationError } =
      await supabaseAdmin()
        .from('whatsapp_webhook_verification')
        .select('token_sha256')
        .eq('active', true);

    if (verificationError) {
      console.error('[webhook] verification configuration unavailable');
    }

    if (
      matchesWebhookVerifyToken(verifyToken, [
        envHash,
        ...(verificationRows ?? []).map(
          (row: { token_sha256: string }) => row.token_sha256
        ),
      ])
    ) {
    ...
    // Backward-compatible verification for fully configured accounts.
