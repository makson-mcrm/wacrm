...
export function toErrorResponse(err: unknown): NextResponse {
  ...
  console.error(
    '[toErrorResponse] uncategorized error',
    safeErrorTelemetry(err)
  );
...
export async function getCurrentAccount(): Promise<AccountContext> {
  ...
  if (error) {
    console.error(
      '[getCurrentAccount] profile fetch error:',
  ...
  if (accountErr) {
    console.error(
      '[getCurrentAccount] account fetch error:',
      safeErrorTelemetry(accountErr)
    );
    throw new ForbiddenError('Could not load account context');
  }
  if (!account) {
