...
export function ContactDetailView({
  ...
  async function insertCallNote(noteText: string) {
  ...
  function startPhoneCall() {
    ...
    void insertCallNote('📞 Rozpoczęto połączenie telefoniczne');
  }
  ...
  async function saveCallOutcome() {
    ...
    setSavingCall(true);
    const details = callNote.trim() ? `\n${callNote.trim()}` : '';
    const saved = await insertCallNote(
      `📞 Wynik rozmowy: ${callOutcome}${details}`
    );

    if (saved) {
      await fetchNotes();
      setCallFollowUpOpen(false);
      toast.success('Wynik rozmowy zapisany');
    } else {
      toast.error('Nie udało się zapisać wyniku rozmowy');
    }
    setSavingCall(false);
  }
  ...
  return (
    <>
      ...
      />
      <Dialog open={callFollowUpOpen} onOpenChange={setCallFollowUpOpen}>
        <DialogContent className="border-border bg-popover text-popover-foreground sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Zapisz wynik rozmowy</DialogTitle>
            <DialogDescription>
              Po zakończeniu połączenia wybierz wynik. Zapis trafi do historii
              kontaktu.
            </DialogDescription>
          </DialogHeader>

          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
            {['Odebrano', 'Nie odebrano', 'Oddzwonić'].map((outcome) => (
              <Button
                key={outcome}
                type="button"
                size="sm"
          ...
          <Textarea
            ...
            placeholder="Krótka notatka po rozmowie (opcjonalnie)"
          ...
          <DialogFooter>
            <Button
              type="button"
              variant="outline"
              onClick={() => setCallFollowUpOpen(false)}
              disabled={savingCall}
            >
              Pomiń
            </Button>
            <Button
              ...
              disabled={savingCall || !callOutcome}
            >
              {savingCall && <Loader2 className="size-4 animate-spin" />}
              Zapisz wynik
            </Button>
          </DialogFooter>
        </DialogContent>
